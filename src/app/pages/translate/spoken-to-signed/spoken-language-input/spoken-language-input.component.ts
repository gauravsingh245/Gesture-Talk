...
import { addOutline, sparkles, language } from 'ionicons/icons';
...
export class SpokenLanguageInputComponent extends BaseComponent implements OnInit {
  ...
  translatedEnglishText$!: Observable<string>;
  ...
  constructor() {
    ...
    this.translatedEnglishText$ = this.store.select<string>(state => state.translate.translatedEnglishText);

    addIcons({ sparkles, addOutline, language });
  }
  ...
  translate() {
    ...
    if (text) {
      this.store.dispatch(new TranslateToEnglish());
    }
  }
}
