...
export class SkeletonPoseViewerComponent extends BasePoseViewerComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    fromEvent(pose, 'firstRender$')
      .pipe(
        tap(async () => {
          const getPoseCanvas = () => new Promise<HTMLCanvasElement>((resolve) => {
            const check = () => {
              const canvas = pose.shadowRoot ? pose.shadowRoot.querySelector('canvas') : null;
              if (canvas) resolve(canvas);
              else requestAnimationFrame(check);
            };
            check();
          });
          const poseCanvas = await getPoseCanvas();
